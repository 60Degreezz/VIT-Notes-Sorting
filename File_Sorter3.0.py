...
2. Change the Downloads dictoinary. Incase your downloaded VTOP files are not stored in the downloads folder please update your Downloads variable below. If your
   Downloads path doesnt match the one given below pls change that too.
...
Sorted_Folder_Location = "F:/"                                  #The folder path where you want to store the sorted files
Downloads = "C:/Users/ADMIN/Downloads"                          #The downloads folder path

def scan_sort():                                                 #This function scans downloads and returns the list of files fitting VIT file format.
    sorted_files = []
    ...
    classified_file_list = divide_and_sort(sorted_files)            #divide and sort function is called so the list is classified
    return classified_file_list

def divide_and_sort(sorted_files):                                       #This function accepts files one by one and makes a class object which has its (Year/Sem/SubCode/Type)
    classified_files = []
    for file in sorted_files:
        ...
        if(file_split[0][0] == 'F'):
            ...
            file_details.append(file_split[0][7:11])
        elif(file[0:6] == 'WINSEM'):
            file_details.append('Win_Sem')
            file_details.append(file_split[0][6:10])
        file_details.append(file_split[1])
        if file_split[2] == 'TH':
